...
let format' = format; in let
  joinlines = (list: lib.lists.foldr (a: b: a + "\n" + b) "" list);
  ...
  prepareImage = ''
    ...
    ${if partitionTableType != "none" then ''
      ...
      mkfs.${fsType} -b ${blockSize} -F -L ${label} $diskImage -E offset=$(sectorsToBytes $START) $(sectorsToKilobytes $SECTORS)K -U ${rootUuid}
    '' else ''
      mkfs.${fsType} -b ${blockSize} -F -L ${label} $diskImage -U ${rootUuid}
    ''}
  ...
  buildImage = pkgs.vmTools.runInLinuxVM (
    pkgs.runCommand name
      {
        preVM = prepareImage;
        buildInputs = with pkgs; [ util-linux e2fsprogs dosfstools zstd ];
